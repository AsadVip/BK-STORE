...
export default function WishlistPage() {
    ...
    const { data: products, isLoading } = useQuery<ProductCardData[]>({
        ...
        queryFn: async () => {
            ...
            (images ?? []).forEach((img) => {
                const i = img as unknown as { product_id: string; url: string; is_primary: boolean };
                if (!imageMap.has(i.product_id) && i.url) imageMap.set(i.product_id, i.url);
            });
            const stockMap = new Map<string, boolean>();
            const variantProductIds = new Set<string>();
            (variants ?? []).forEach((v) => {
                const vr = v as unknown as { product_id: string; stock_quantity: number };
                variantProductIds.add(vr.product_id);
                if (vr.stock_quantity > 0) stockMap.set(vr.product_id, true);
            });
        ...
        <div className="container-bk py-12">
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-2 gap-6 md:grid-cols-3 lg:grid-cols-4">
                    {products.map((p) => (
                        <ProductCard key={p.id} product={p} isWishlisted onToggleWishlist={toggle} />
                    ))}
                </div>
            ) : (
                <EmptyState
